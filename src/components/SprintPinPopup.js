...
const SprintPinPopup=(props)=> {
  ...
  const onProceed= async()=>{
     const isPinCorrect = await confirmPin()
     
     if(isPinCorrect){
      props.confirmationSprintPin()
     }else{
  ...
  const confirmPin=async()=>{
    ...
    var responseRecieved = await Method.confirmSprintEdit(props.selectedProject.id, enteredPin)
  ...
  return (
    <div className="w-100p">
      <div id="SprintPinPopup" className="modal">
          <div className="modal-content">
            
            <h5>ACCESS CONFIRMATION</h5>
            <p>Enter project pin to proceed</p>
